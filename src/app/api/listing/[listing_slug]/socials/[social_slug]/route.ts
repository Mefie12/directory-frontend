...
export async function PUT(
  request: NextRequest,
  { params }: { params: Promise<{ listing_slug: string; social_slug: string }> }
) {
  try {
    const { listing_slug, social_slug } = await params;
    const body = await request.json();
    const authHeader = request.headers.get('Authorization');

    const response = await fetch(
      `${API_BASE_URL}/api/listing/${listing_slug}/socials/${social_slug}`,
      {
        method: 'PUT',
        headers: {
