...
export async function PUT(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/listing/${listing_slug}/address`,
      {
        method: 'PUT',
        headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: errorData.message || 'Failed to update listing address' },
    ...
    console.error('Error updating listing address:', error);
    return NextResponse.json(
