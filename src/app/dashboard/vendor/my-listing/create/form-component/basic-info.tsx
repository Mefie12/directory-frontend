...
export function BasicInformationForm({ form, listingType, onSubmit }: Props) {
    ...
    const urlType = searchParams.get('type') as "business" | "event" | "community" | null;
    ...
    const handleMainCategoryChange = (categoryId: string, category?: Category) => {
        const selectedCategory = category || categories.find(cat => cat.id === categoryId);
    ...
    const handleSubcategoryClick = (subCategoryId: string) => {
        ...
        const allCategoryIds = [selectedMainCategoryId, ...newSelectedSubCategories];
    ...
    const handleFormSubmit = async (data: BusinessFormValues) => {
        try {
            ...
            const response = await fetch(`${API_URL}/api/listings/profile`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
    ...
    return (
        <form onSubmit={handleSubmit(handleFormSubmit)} className="w-full max-w-5xl space-y-6 mx-auto p-6">
            <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-1">
                    ...
                    <Input
                        ...
                        className={cn(
                            "h-10 rounded-lg border-gray-300 px-4 text-gray-800 placeholder:text-gray-400 focus-visible:ring-2 focus-visible:ring-black",
                            errors.primary_phone && "border-red-500 focus-visible:ring-red-500"
                        )}
                    />
                ...
                <div className="space-y-1">
                    <label className="font-medium text-sm">Secondary Phone (Optional)</label>
                    <Input
                ...
                {selectedMainCategoryId && subCategories.length > 0 && (
                    <div className="space-y-3">
                        <div className="flex items-center justify-between">
                            <label className="font-medium text-sm">Subcategories (Optional)</label>
                        ...
                        <div className="flex items-center justify-between">
                            <p className="text-xs text-gray-500">
                                Click to select/deselect subcategories. Main category is always included.
                            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {listingType === "business" && (
                    <div className="space-y-1">
                        <label className="font-medium text-sm">Business Registration Number (Optional)</label>
                        <Input
            ...
            {submitSuccess && (
                <div className="p-4 bg-green-50 border border-green-200 text-green-700 rounded-lg">
                    ...
                    <p className="text-sm mt-1">Your listing has been created successfully. Redirecting...</p>
                </div>
            )}
