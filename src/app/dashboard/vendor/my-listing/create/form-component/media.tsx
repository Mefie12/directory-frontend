...
export const MediaUploadStep = forwardRef<ListingFormHandle, Props>(
    ({ listingType, listingSlug }, ref) => {
        ...
        const uploadWithChunking = async () => {
            ...
            if (totalFiles < 4) {
                toast.error(`Please upload 4 media files total (1 cover + 3 gallery). Currently: ${totalFiles}`);
                return false;
            }
            ...
            try {
                ...
                allFiles.forEach((file, i) => {
                    console.log(`${i}: ${file.name} - ${(file.size/1024/1024).toFixed(2)}MB - ${file.type}`);
                });
                ...
                const totalSizeMB = optimizedFiles.reduce((sum, file) => sum + file.size, 0) / 1024 / 1024;
                ...
                const attemptBulkUpload = async (files: File[]): Promise<boolean> => {
                    try {
                        ...
                        const response = await fetch(`${API_URL}/api/listing/${listingSlug}/media`, {
                            method: "POST",
                            headers: {
                ...
                const uploadChunked = async (files: File[]): Promise<boolean> => {
                    ...
                    for (let i = 0; i < files.length; i += CHUNK_SIZE) {
                        ...
                        toast.loading(`Uploading chunk ${chunkNumber} of ${totalChunks}...`);
                        ...
                        try {
                            ...
                            if (!response.ok) {
                                const error = await response.json();
                                throw new Error(`Chunk ${chunkNumber} failed: ${error.message || "Unknown error"}`);
                            }
            ...
            <div className="p-6 space-y-6">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Upload 4 media files (images or videos). First file will be the cover.
                    </p>
                ...
                <div className="space-y-6">
                    ...
                    <div className="pt-4 border-t">
                        ...
                        <div className="text-sm space-y-1">
                            <div className="flex justify-between">
                                ...
                                <span className={media.coverPhoto ? "text-green-600" : "text-red-600"}>
                   {media.coverPhoto ? "✓ Ready" : "✗ Missing"}
                                    {media.coverPhoto && ` (${(media.coverPhoto.size/1024/1024).toFixed(1)}MB)`}
                 </span>
                            </div>
                            <div className="flex justify-between">
                                <span>Gallery media:</span>
                                <span className={media.images.length === 3 ? "text-green-600" : "text-amber-600"}>
                   {media.images.length}/3 files
                                    {media.images.length > 0 && ` (${(media.images.reduce((sum, f) => sum + f.size, 0)/1024/1024).toFixed(1)}MB total)`}
                 </span>
                            </div>
                   ...
                   {[media.coverPhoto, ...media.images].length}/4 files
                                    {media.coverPhoto && ` (${([media.coverPhoto, ...media.images].reduce((sum, f) => sum + f.size, 0)/1024/1024).toFixed(1)}MB)`}
                 </span>
                            </div>
                            {isUploading && (
                                <div className="mt-2 text-blue-600 text-xs">
                                    ⏳ Uploading files (will try bulk first, then chunks if needed)...
                                </div>
                            )}
                        </div>
                    </div>
