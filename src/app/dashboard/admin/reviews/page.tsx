...
export default function ReviewsPage() {
    ...
    // Cache for users and listings to avoid duplicate API calls
    const [usersCache, setUsersCache] = useState<Map<string, User>>(new Map());
    const [listingsCache, setListingsCache] = useState<Map<string, Listing>>(new Map());
    ...
    const fetchUser = async (userId: string): Promise<User> => {
        ...
        try {
            ...
            // Format user name
            const userName = user.name || `${user.first_name || ''} ${user.last_name || ''}`.trim() || 'Anonymous User';
            ...
            // Update cache
            setUsersCache(prev => new Map(prev.set(userId, userObj)));
    ...
    const fetchListing = async (listingId: string): Promise<Listing> => {
        // Check cache first
        if (listingsCache.has(listingId)) {
            return listingsCache.get(listingId)!;
        }
        ...
        try {
            ...
            const listing = listingData.data || listingData.listing || listingData;

            const listingName = listing.name || listing.title || listing.business_name || 'Unknown Listing';
            ...
            // Update cache
            setListingsCache(prev => new Map(prev.set(listingId, listingObj)));
    ...
    // --- Helper: Data Mapping ---
    const extractReviewsFromResponse = async (apiData: unknown): Promise<Review[]> => {
        ...
        // Process reviews and fetch user/listing details in parallel
        const reviewPromises = rawItems.map(async (item) => {
            const userId = item.user_id.toString();
            const listingId = item.listing_id.toString();

            // Fetch user and listing details in parallel
            ...
            return {
                ...
                vendor: "Vendor Name", // You might need to fetch this separately from listing data
                review: item.comment || "No content provided",
                rating: item.rating || 0,
                dateSubmitted: item.created_at
                    ? new Date(item.created_at).toLocaleDateString()
                    : "N/A",
                status: (item.status as "Published" | "Flagged" | "Pending") || "Published",
            };
        });
